...
int DB[] = {4, 7, 8, 9};  //wire these to DB4~7 on LCD.
...
void LCD4Bit_mod::pushNibble(int value){
  int val_nibble= value & 0x0F;  //clean the value.  (unnecessary)
   ...
   digitalWrite(DB[0],val_nibble & 01);
   val_nibble >>= 1;
   digitalWrite(DB[1],val_nibble & 01);
   val_nibble >>= 1;
   
   digitalWrite(DB[2],val_nibble & 01);
   val_nibble >>= 1;
   digitalWrite(DB[3],val_nibble & 01);
   val_nibble >>= 1;
  /*  
