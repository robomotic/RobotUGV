...
int main()
{
	...
	while (1) // Loop forever
	{
		...
		else
		{
			 ...
			 *
			 */
			long m1temp = (neutralPulseTime - (int)ch[0].pulse) + ((int)ch[1].pulse - neutralPulseTime);
			long m2temp = (neutralPulseTime - (int)ch[0].pulse) - ((int)ch[1].pulse - neutralPulseTime);
			m1 = m1temp * 255 / minPulseTime;
			m2 = m2temp * 255 / minPulseTime;
			
			if(m1>=-250 && m1<=250) set_m1_speed(m1);
			if(m2>=-250 && m2<=250) set_m2_speed(m2);
			//set_motors(m1, m2);
		}
	}
